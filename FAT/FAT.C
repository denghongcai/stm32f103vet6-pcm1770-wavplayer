...
{  		   
	...
	if(SD_ReadBlock(0,(u32*) &fat_buffer[0],512)!=SD_OK)return 2;
	...
	{
		if(SD_ReadBlock(pr->prStartLBA,(u32*) &fat_buffer[0],512)!=SD_OK)return 3;//read the bpb sector
		bs = (bootsector710 *)fat_buffer;
		if(bs->bsJump[0]!=0xE9 && bs->bsJump[0]!=0xEB)
		{
			hidsec = 0;
			if(SD_ReadBlock(0,(u32*) &fat_buffer[0],512)!=SD_OK)return 4;//read the bpb sector
			bs = (bootsector710 *)fat_buffer;	
		}
	}	  
...
unsigned long FAT_NextCluster(unsigned long cluster)
{
	...
	if(SD_ReadBlock(sector,(u32*) &fat_buffer[0],512)!=SD_OK)return 0x0ffffff8;//读取FAT表,发生错误是返回0x0ffffff8
	if(FAT32_Enable)
	{
...
{ 			  	   
	...
	{			 
		...
		{
			if(SD_ReadBlock(FirstDirSector+cnt,(u32*) &fat_buffer[0],512)!=SD_OK)return 0;//读数错误    	   
	...
	else//其他文件夹/FAT32系统
	{
		...
		while(1)
		{
			...
			{
				if(SD_ReadBlock(sector+cnt,(u32*) &fat_buffer[0],512)!=SD_OK)return 0;
				for(offset=0;offset<512;offset+=32)
				{
...
{	
	...
	if(SD_ReadBlock(sector+FileInfo->F_Offset,(u32*) &buf[0],512)!=SD_OK)return 0;//读数错误   
	FileInfo->F_Offset++;
...
FileInfoStruct F_Search(u32 cluster,unsigned char *Name,u32 type)
{
	...
	{			 
		for(cnt=0;cnt<RootDirSectors;cnt++)
		{
			if(SD_ReadBlock(FirstDirSector+cnt,(u32*) &fat_buffer[0],512)!=SD_OK)
			{
	...
	}else//其他文件夹/FAT32系统
	{
		...
		while(1)
		{
			...
			for(cnt=0;cnt<SectorsPerClust;cnt++)
			{
				if(SD_ReadBlock(sector+cnt,(u32*) &fat_buffer[0],512)!=SD_OK)
				{
					FileInfo.F_StartCluster=0;//读数错误 
					return FileInfo;   
				}
